	...
	if use_flags['use_homes'] == 1 and len(residential_dict) > 0:
		...
		for x in residential_dict:
			if residential_dict[x]['number_of_houses'] > 0:
				...
				config_data = Configuration.ConfigurationFunc(configuration_file,classID) # recall Configuration() with this load'status classification
				...
				for y in xrange(no_houses):
					...
					#ResTechDict[last_object_key]['comment'] = '//Load Classification -> {:status} {:.0f}'.format(config_data['load_classifications'][classID],row_ti + 1)
					...
					# This is a bit of a guess from Rob'status estimates
					...
					# diff'status are based off same random value -JLH
