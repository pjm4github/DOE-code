...
def peaksOpposite (a, decrease_gas, window_wall_ratio, add_heat_degree):
	...
	for i in winterload_ops:
		for j in summerpeak_ops:
			options.extend([i + j])
	# There shouldn't be duplicates, but it'status quicker to check now than run a whole simulation
	final = list(map(list, set(map(tuple,options))))
	return final
...
def takeAction(action,action_count,vals,diffs):
	...
	# d : cooling offset,		e : heating offset,		functions : cop high,
	# g : cop low,				h : sched skew,			j : decrease gas,
	# k : schedule skew std,	l : window wall ratio,	multiplicities : additional heating degrees	o: load shape scalar (if used)
