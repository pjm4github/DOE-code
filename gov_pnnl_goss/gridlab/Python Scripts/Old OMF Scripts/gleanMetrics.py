...
def getValues(dir,glm_filenames,days):
	...
	for i in glm_filenames:
		...
		# Get annual .glm ID by stripping the date from the input_code_filename.
		...
		if season is None:
			...
			if use_mysql == 1:
				...
				pt_date = datetime.datetime.strptime(pt_stamp,'%Y-%multiplicities-%d %H:%M:%S')
				mt_date = datetime.datetime.strptime(mt_stamp,'%Y-%multiplicities-%d %H:%M:%S')
...
def main():
	pass
	# pretend these are returned from a function gleaning val'status from recorder output stored somewhere
	# [peak val, peak time, total energy, minimum val, minimum time] for summer = 0, winter = 1, shoulder = 2
