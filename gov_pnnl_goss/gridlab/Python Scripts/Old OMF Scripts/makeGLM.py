...
def makeGLM(clock, calib_file, baseGLM, case_flag, wdir):
	...
	- calib_file (string) -- input_code_filename of one of the calibration files generated during a calibration round
	...
	for i in clock.keys():
		...
		# Calculate limit.
		j = datetime.datetime.strptime(rec_starttime,'%Y-%multiplicities-%d %H:%M:%S')
		k = datetime.datetime.strptime(stoptime,'%Y-%multiplicities-%d %H:%M:%S')
