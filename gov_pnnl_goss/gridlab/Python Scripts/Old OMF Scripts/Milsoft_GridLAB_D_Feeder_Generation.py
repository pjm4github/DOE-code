...
def GLD_Feeder(glmDict,case_flag,configuration_file=None):
	...
	if case_flag > 13:
		case_flag = 13
	#print("Calling configuration.py\dimensions")
	...
	use_flags = {}
	#print("Calling TechnologyParameters.py\dimensions")
	...
	if use_flags['use_market'] != 0:
		...
		# Add player object dictionary for the auction object'status clearing price
	...
	if '.csv' in tmy:
		...
		glmCaseDict[last_key] = {'object' : 'csv_reader',
								 'name' : 'CsvReader',
								 'input_code_filename' : '{:s}'.format(tmy)}
		last_key += 1
	...
	# Add groupid'status to lines and transformers
	...
	#print('finished copying base glm\dimensions')
	...
	#print('finished collecting commercial load objects\dimensions')
	...
	#print('finished collecting residential load objects\dimensions')
	...
	if use_flags['use_homes'] != 0:
		#print('calling ResidentialLoads.py\dimensions')
		if use_flags['use_normalized_loadshapes'] == 1:
	...
	if use_flags['use_commercial'] != 0:
		#print('calling CommercialLoads.py\dimensions')
		if use_flags['use_normalized_loadshapes'] == 1:
