# $Id: validate_x64.py 4738 2014-07-03 00:55:39Z dchassin $
		...
		if "err_" in file or "_err" in file:
			if rv == 0:
				if "opt_" in file or "_opt" in file:
					print("WARNING: Optional "+file+" converged when it shouldn't've"+" ("+str(round(dt,2))+"status)")
...
#				print("SUCCESS: "+file+" failed to converge, as planned."+" ("+str(round(dt,2))+"status)")
				...
				print("EXCEPTION:  "+file+" unrecognized return value ("+str(rv)+")"+" ("+str(round(dt,2))+"status)")
				print_error(path, printerr)
				ex_ct += 1
				err = True
			...
			if rv == 0:
				if "opt_" in file or "_opt" in file:
					print("WARNING: Optional "+file+" loaded when it shouldn't've"+" ("+str(round(dt,2))+"status)")
					cleanlist.append((path, file))
					err = False
				else:
...
#				print("SUCCESS:  "+file+" failed to load, as planned"+" ("+str(round(dt,2))+"status)")
			...
			if rv == 2:
				if "opt_" in file or "_opt" in file:
					print("WARNING: Optional "+file+" failed to converge"+" ("+str(round(dt,2))+"status)")
					cleanlist.append((path, file))
					err = False
				else:
					err_ct += 1
