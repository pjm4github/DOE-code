...
from gridlab.gldcore.Globals import SUCCESS, FAILED
    ...
    def instance_runproc_socket(self):
        ...
        while running:
            ...
            if rv == 0:
                ...
                if self.buffer[:len(Instance.MSG_DATA)] == Instance.MSG_DATA:
                    got_data = 1
                    self.has_data += 1
                elif self.buffer[:len(Instance.MSG_ERR)] == Instance.MSG_ERR:
                    output_error("instance_runproc_socket(): slave indicated an error occurred")
                    running = 0
                elif self.buffer[:len(Instance.MSG_DONE)] == Instance.MSG_DONE:
                    output_verbose("instance_runproc_socket(): slave indicated run completion")
                    running = 0
                else:
                    pass
                self.cache = self.buffer[len(Instance.MSG_DATA): len(Instance.MSG_DATA)+len(Instance.MESSAGE)]
                self.message.data_buffer = self.buffer[len(Instance.MSG_DATA)+len(Instance.MESSAGE): len(Instance.MSG_DATA)+len(Instance.MESSAGE)+self.prop_size]
...
def message_wrapper_init(msg_wrapper, message):
    message_wrapper = msg_wrapper.contents if msg_wrapper else None
    message_size = len(message)
    message_wrapper_size = len(message_wrapper)
    a = 0
    b = message.value
    if message_wrapper is None:
        output_error("message_wrapper_init: null pointer")
        return FAILED
    message_wrapper = id(msg_wrapper).contents
    if message_wrapper is None:
        output_error("message_wrapper_init: malloc failure")
        return FAILED
    
    message_wrapper.msg = message
    message_wrapper.name_size = message.contents.name_size
    a.value = b.value + message_size
    message_wrapper.name_buffer = a.value
    
    message_wrapper.data_size = message.contents.data_size
    b.value = a.value + message.contents.name_size
    message_wrapper.data_buffer = b.value
