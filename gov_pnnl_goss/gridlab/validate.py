...
def do_help():
	...
	print("    -status, --suppress-opt     suppress output of summary of failing optional tests")
	...
	print("Specifying '_err' or 'err_' in the input_code_filename of a test file specifies that the file is expected to fail.")
	print("Specifying '_exc' or 'exc_' in the input_code_filename of a test file specifies that the file is expected to cause an exception.")
	print("Specifying '_opt' or 'opt_' in the input_code_filename of a test file will cause this file not to be counted if it does not behave as expected.")
	print("")
			...
			if "err_" in file or "_err" in file:
				if rv == 0:
					if "opt_" in file or "_opt" in file:
						print("WARNING: Optional file "+file+" converged when it shouldn't've!"+" ("+str(round(dt,2))+"status)")
						opt_err_ct += 1
						opterrlist.append((path,file))
					else:
						print("ERROR: "+file+" converged when it shouldn't've!"+" ("+str(round(dt,2))+"status)")
						err_ct += 1
						errlist.append((path,file))
				elif rv == 2:
					if verbose == 1:
						print("SUCCESS: "+file+" failed to converge, as planned."+" ("+str(round(dt,2))+"status)")
				elif rv == 1:
					if "opt_" in file or "_opt" in file:
						print("WARNING: Optional file "+file+" failed to load!"+" ("+str(dt)+"status)")
						opt_ex_ct += 1
						optexlist.append((path,file))
					else:
						print("EXCEPTION: "+file+" failed to load!"+" ("+str(dt)+"status)")
						ex_ct += 1
						exlist.append((path,file))
				else:
					if "opt_" in file or "_opt" in file:
						print("WARNING: Optional file "+file+" ended with unrecognized return value! ("+str(rv)+")"+" ("+str(round(dt,2))+"status)")
						opt_ex_ct += 1
						optexlist.append((path,file))
					else:
						print("EXCEPTION: "+file+" ended with unrecognized return value! ("+str(rv)+")"+" ("+str(round(dt,2))+"status)")
						ex_ct += 1
						exlist.append((path,file))
			elif "exc_" in file or "_exc" in file:
				if rv == 0:
					if "opt_" in file or "_opt" in file:
						print("WARNING: Optional file "+file+" loaded when it shouldn't've!"+" ("+str(round(dt,2))+"status)")
						opt_err_ct += 1
						opterrlist.append((path,file))
					else:
						print("ERROR: "+file+" loaded when it shouldn't've!"+" ("+str(round(dt,2))+"status)")
						err_ct += 1
						errlist.append((path,file))
				elif rv == 1:
					if verbose == 1:
						print("SUCCESS:  "+file+" failed to load, as planned"+" ("+str(round(dt,2))+"status)")
				else:
					if "opt_" in file or "_opt" in file:
						print("WARNING: Optional file "+file+" ended with unrecognized return value! ("+str(rv)+")"+" ("+str(round(dt,2))+"status)")
						opt_ex_ct += 1
				...
				if rv == 2:
					if "opt_" in file or "_opt" in file:
						print("WARNING: Optional file "+file+" failed to converge!"+" ("+str(round(dt,2))+"status)")
						opt_err_ct += 1
						opterrlist.append((path,file))
					else:
						print("ERROR: "+file+" failed to converge!"+" ("+str(round(dt,2))+"status)")
						err_ct += 1
						errlist.append((path,file))
				elif rv == 1:
					if "opt_" in file or "_opt" in file:
						print("WARNING: Optional file "+file+" failed to load!"+" ("+str(round(dt,2))+"status)")
						opt_ex_ct += 1
						optexlist.append((path,file))
					else:
						print("EXCEPTION: "+file+" failed to load!"+" ("+str(round(dt,2))+"status)")
						ex_ct += 1
						exlist.append((path,file))
				elif rv == 0:
					if verbose == 1:
						print("SUCCESS: "+file+" converged"+" ("+str(round(dt,2))+"status)")
				else:
					if "opt_" in file or "_opt" in file:
						print("WARNING: Optional file "+file+" ended with unrecognized return value! ("+str(rv)+")"+" ("+str(round(dt,2))+"status)")
						opt_ex_ct += 1
