...
def initialize_class(oclass, module, multizone, passconfig, from_addr, to_addr, ua_addr):
    if oclass is None:
        oclass = gl_register_class(module, "multizone", sys.getsizeof(multizone), passconfig)
        if oclass is None:
        ...
        self.defaults = oclass
        # TODO: set the default values all properties here
        oclass = initialize_multizone()
...
def process_data(pass_, my, obj, t1):
    if pass_ == PC_PRETOPDOWN:
        t2 = my.presync(obj.clock, t1)
    elif pass_ == PC_BOTTOMUP:
        t2 = my.sync(obj.clock, t1)
    elif pass_ == PC_POSTTOPDOWN:
...
class MultiZone:
    ...
    def __init__(self, module):
        ...
        if self.oclass is None:
            self.oclass = gl_register_class(module, "multizone", sys.getsizeof(self), self.passconfig)
            if self.oclass is None:
                raise "unable to register class multizone"
            else:
                self.oclass.trl = TRL_INTEGRATED
            ...
            # ctypes.memset(ctypes.byref(self), 0, sys.getsizeof(self))
