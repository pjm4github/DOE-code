...
ins_pec_template = """
 ...
 <{url}#{res}> c:PowerElectronicsConnection.precisions \"{precisions}\".
...
ins_syn_template = """
 ...
 <{url}#{res}> c:Equipment.EquipmentContainer  <{url}#{resFdr}>.
 <{url}#{res}> c:PowerSystemResource.Location <{url}#{resLoc}>.
 <{url}#{res}> c:SynchronousMachine.precisions \"{precisions}\".
...
ins_pep_template = """
 ...
 <{url}#{res}> c:PowerElectronicsConnectionPhase.precisions \"{precisions}\".
    ...
    if qbus is not None:    # have retrieved essential circuit information
        if not toks[0].startswith('//') and len(toks[0]) > 0:
            ...
            if unit == "SynchronousMachine":
                ...
                inssyn = ins_syn_template.format(url=CIMHubConfig.blazegraph_url, res=idSYN, nm=name, resLoc=idLoc,
                                                 resFdr=fdr_id, resUnit=idUnit,
                ...
                inspec = ins_pec_template.format(url=CIMHubConfig.blazegraph_url, res=idPEC, nm=name, resLoc=idLoc,
                                                 resFdr=fdr_id, resUnit=idUnit,
                                                 p=kW*1000.0, q=kVAR*1000.0, ratedS=kVA*1000.0, ratedU=kV*1000.0)
                ...
                if len(phases) > 0 and phases != 'ABC':
                    ...
                    for phs in phase_list:
                        ...
                        inspep = ins_pep_template.format (url=CIMHubConfig.blazegraph_url, res=idPhs, nm=nmPhs,
                                                          resPEC=idPEC, resLoc=idLoc, ns=CIMHubConfig.cim_ns,
                                                          phs=phs, p=p, q=q)
                ...
                if unit == 'Battery':
                    ...
                    if kW > 0.0:
                        ...
                        state = 'Charging'
                    insunit = ins_bat_template.format(url=CIMHubConfig.blazegraph_url, res=idUnit, nm=nmUnit,
                                                      resLoc=idLoc, ns=CIMHubConfig.cim_ns,
                                                      ratedE=ratedkwh*1000.0, storedE=storedkwh*1000.0, state=state)
                elif unit == 'Photovoltaic':
                    insunit = ins_pv_template.format(url=CIMHubConfig.blazegraph_url, res=idUnit, nm=nmUnit,
                                                     resLoc=idLoc)
                else:
            ...
            if unit == "SynchronousMachine":
                instrm = ins_trm_template.format(url=CIMHubConfig.blazegraph_url, res=idTrm, nm=nmTrm, resCN=idCN,
                                                 resEQ=idSYN)
            else:
                instrm = ins_trm_template.format(url=CIMHubConfig.blazegraph_url, res=idTrm, nm=nmTrm, resCN=idCN,
                                                 resEQ=idPEC)
