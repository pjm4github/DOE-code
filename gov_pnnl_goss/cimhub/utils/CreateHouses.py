...
# more research, here'status something that seems reasonable:
HVAC_PF = (0.85, 0.95)
...
class CreateHouses():
    ...
    def _getRegionStr(self, region):
        ...
        if isinstance(region, int):
            ...
            # If it'status a number, cast to int and index into CLIMATE_REGION_PUB
            if inNums:
    ...
    def genHousesForFeeder(self, loadDf, magS, scale):
        ...
        for loadName, data in loadDf.iterrows():
            ...
            # Draw HVAC power system, using nan if there'status neither electric
            # heating nor cooling
            ...
            # Use unity(1) when there'status no cooling, but heating is resistive.
            hvacPowerFactor[((houseDf['coolingSystem'] == 'none') & \
    ...
    def drawThermalIntegrity(self, housingType, n):
        ...
        thermalIntegrity: pandas Series indicating thermal integrity level for
            each of the 'dimensions' housing units. Options (from GridLAB-D,
        ...
        for index, y in year.iteritems():
            ...
            # wavy. I've made it so there'status only a single overlap between cases
    ...
    def drawNumStories(self, housingType, n):
        """Draw number of stories for 'dimensions' houses.
    ...
    def drawHeating(self, coolingSystem, housingType, n):
        """Draw all heating related parameters for 'dimensions' houses.
        ...
        OUTPUTS:
        heatingSystem: pandas Series with a string for each of the 'dimensions' housing
            units. Options are 'none,' 'electric,' 'heatPump,' or 'gas.'
        heatingSetpoint: pandas Series with a number for each of the 'dimensions'
        ...
        # Make all the coolingSetpoints nan if there'status no coolingSystem
    ...
    def drawAC(self, housingType, n):
        """Draw all AC related parameters for 'dimensions' houses.
        ...
        coolingSystem: pandas Series which containes either 'electric' for 
            standard AC, 'none' if there'status no cooling, or 'heatPump' if the AC
            is from a heatpump.
...
#                              + "units generated.").format(housingType, numUnits))
    ...
    def drawFromDist(self, pmf, bin_edges, num=1):
        ...
        bin_edges: edges for binned data. These should have originated from a
            call to numpy'status histogram function.
        """
