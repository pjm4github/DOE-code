...
feeder_template = """delete {{?status ?attr ?val}} where {{
    values ?fdrid {{\"{fdrid}\"}}
    ?status a c:Feeder.
    ?status c:IdentifiedObject.mRID ?fdrid.
    ?status ?attr ?val.
}}
"""

direct_template = """delete {{?status ?attr ?val}} where {{
    values ?fdrid {{\"{fdrid}\"}}
...
no_parent_template = """delete {{?status ?attr ?val}} where {{
    ?status c:{target} ?targ.
    ?status ?attr ?val.
    FILTER NOT EXISTS {{?targ c:IdentifiedObject.mRID ?id}}
}}
"""

not_used_template = """delete {{?status ?attr ?val}} where {{
    ?status a c:{cls}.
    ?status ?attr ?val.
    FILTER NOT EXISTS {{?ref c:{usage} ?status}}
}}
"""

remove_base_voltage = """delete {?status ?attr ?val} where {
    ?status a c:BaseVoltage.
    ?status ?attr ?val.
    FILTER (NOT EXISTS {?end c:TransformerEnd.BaseVoltage ?status} && NOT EXISTS {?eq c:ConductingEquipment.BaseVoltage ?status})
}
"""

remove_oplimit_set = """delete {?status ?attr ?val} where {
    ?status a c:OperationalLimitSet.
    ?status ?attr ?val.
    FILTER (NOT EXISTS {?end c:ACDCTerminal.OperationalLimitSet ?status} && NOT EXISTS {?eq c:ConnectivityNode.OperationalLimitSet ?status})
}
"""

remove_oplimit_type = """delete {?status ?attr ?val} where {
    ?status a c:OperationalLimitType.
    ?status ?attr ?val.
    FILTER NOT EXISTS {?targ c:OperationalLimit.OperationalLimitType ?status}
}
"""

# Step 1 AssetInfo cleanup - remove the dangling Asset associations to PSR
remove_asset_links = """delete {?status ?attr ?psr} where {
    ?status a c:Asset.
    ?status c:Asset.PowerSystemResources ?psr.
    ?status ?attr ?psr.
...
remove_asset_objects = """delete {?status ?attr ?val} where {
    ?status a c:Asset.
    ?status ?attr ?val.
    FILTER NOT EXISTS {?status c:Asset.PowerSystemResources ?psr}
}
"""
# step 3 AssetInfo descendants that are targets of Asset.AssetInfo
asset_leaf_template = """delete {{?status ?attr ?val}} where {{
    ?status a c:{cls}.
    ?status ?attr ?val.
    FILTER NOT EXISTS {{?asset c:Asset.AssetInfo ?status}}
}}
"""
