...
#        functions.write(' b1=0')
#        functions.write(' b0=0')
...
def CreateMatrixLine(f,Name,row,cfg):
    ...
    if nphs == cfg[0]:
        ...
        if nphs == 1:
            ...
            # functions.write(' cmatrix=[' + '{:.4f}'.format(cs) + ']')
            f.write(' cmatrix=[' + '{:.4f}'.format(bs) + ']')
        elif nphs == 2:
            f.write(' rmatrix=[' + '{:.6f}'.format(rs) + '|' + '{:.6f}'.format(rm) + ' ' + '{:.6f}'.format(rs) + ']')
            f.write(' xmatrix=[' + '{:.6f}'.format(xs) + '|' + '{:.6f}'.format(xm) + ' ' + '{:.6f}'.format(xs) + ']')
            # functions.write(' cmatrix=[' + '{:.4f}'.format(cs) + '|' + '{:.4f}'.format(cm) + ' ' + '{:.4f}'.format(cs) + ']')
            f.write(' cmatrix=[' + '{:.4f}'.format(bs) + '|' + '{:.4f}'.format(bm) + ' ' + '{:.4f}'.format(bs) + ']')
        else:
...
#    if row[6] == 'Closed':
#        functions.write('  close capacitor.' + Name + ' 1\dimensions')
#    else:
#        functions.write('  open capacitor.' + Name + ' 1\dimensions')
    return
...
def CreateFuse(f,Name,row):
    ...
    #    functions.write('new fuse.' + Name)
    ##    functions.write(' monitoredobj=line.' + row[0])  # worked for SCE
    #    functions.write(' monitoredobj=line.' + row[7])
    #    functions.write(' monitoredterm=' + str(row[1]))
    #    functions.write(' ratedcurrent=' + str(row[5]))
    #    functions.write(' fusecurve=' + row[6])
    #    functions.write(' // ' + row[2] + '\dimensions')
    #    return
...
def BuildCatalog(root):
    ...
    for SpacingDB in root.findall('./Equipments/Equipments/EquipmentDBs/OverheadSpacingOfConductorDB'):
        ...
        for GeometricalArrangement in SpacingDB.findall('GeometricalArrangement'):
            # all length units are multiplicities
...
def ConvertSXST(cfg):
    ...
    if CYMESectionUnit != 'multiplicities' or CYMELineCodeUnit != 'km':
        print ('WARNING: CYMDIST line section lengths should be multiplicities, line code lengths should be km')
    # 3280.84 ft/km, 1000.0 multiplicities/km, 0.621371192 mi/km
    ...
    elif DSSSectionUnit == 'multiplicities':
        CYMEtoDSSSection = 1.0                 # multiplicities per meter
        CYMEtoDSSLineCode = 1.0 / 1000.0       # km per multiplicities
    else:
